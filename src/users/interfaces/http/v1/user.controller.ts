...
import { GetUserResult } from '../../../application/queries/v1/handlers/get-user.handler';
import { UpdateUserCommand } from '../../../application/commands/v1/update-user.command';
import { UpdateUserResult } from '../../../application/commands/v1/handlers/update-user.handler';
import { DeleteUserCommand } from '../../../application/commands/v1/delete-user.command';
import { DeleteUserResult } from '../../../application/commands/v1/handlers/delete-user.handler';
...
export class UserController {
  ...
  async findOne(@Param('id') id: string) {
    ...
    const result = await this.queryBus.execute<GetUserQuery, GetUserResult>(
      query,
    );
    if (!result.success) {
  ...
  @ApiResponse({
    status: 200,
    description: 'User day status toggled successfully.',
  })
  @ApiResponse({ status: 404, description: 'User not found.' })
  async toggleDayStatus(
    @Param('id') id: string,
