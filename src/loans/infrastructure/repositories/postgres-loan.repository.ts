...
interface LoanRawResult {
  installmentsSum: string | number;
  paidToday: string | number;
  inIntervalPayment: string | number;
}

interface DashboardKpisRaw {
  totalLentToday: string | number;
  collectedToday: string | number;
  totalExpensesToday?: string | number; // Added possible field or keep consistent
  collectedTodayYape: string | number;
  collectedTodayCash: string | number;
  activeClients: string | number;
}

interface DashboardExpensesRaw {
  totalExpenses: string | number;
}

interface GlobalStatsRaw {
  historicalCollected: string | number;
}

interface GlobalLentRaw {
  historicalLent: string | number;
}
...
export class PostgresLoanRepository implements LoanRepository {
  ...
  async findAllWithFilters(
    ...
    const todayEndSql =
      "DATE_TRUNC('day', NOW() AT TIME ZONE 'America/Lima') + INTERVAL '1 day'";
  ...
  async findWithInstallments(id: string): Promise<Loan | null> {
    ...
    if (entity.installments) {
      loan.installments = entity.installments.map(
        (inst) =>
          new LoanInstallment(
            inst.loanId,
  ...
  async getDashboardStats(
    ...
    const kpisQb = this.typeOrmRepository.manager
      ...
      .where("loan.status != 'Eliminado'");

    if (userId) {
      kpisQb.andWhere('loan.user_id = :userId', { userId });
    }
    if (companyId) {
      kpisQb.andWhere('u.id_company = :companyId', { companyId });
    }

    const kpisRaw = await kpisQb.getRawOne<DashboardKpisRaw>();
    ...
    const pendingRaw = await pendingQb.getRawAndEntities<LoanRawResult>();

    const pendingLoans = pendingRaw.entities.map((entity, index) => {
      const loan = this.toDomain(entity);

      const installmentsSum = Number(
        pendingRaw.raw[index].installmentsSum ?? 0,
      );
      loan.remainingAmount = loan.amount + loan.interest - installmentsSum;
    ...
    const expensesQb = this.typeOrmRepository.manager
      ...
      .where(
        `expense.expense_date >= ${todayStartSql} AND expense.expense_date < ${todayEndSql}`,
      )
    ...
    const expensesRaw = await expensesQb.getRawOne<DashboardExpensesRaw>();
    ...
    const globalStatsQb = this.typeOrmRepository.manager
      .createQueryBuilder()
      .select([`COALESCE(SUM(installment.amount), 0) AS "historicalCollected"`])
    ...
    const globalLentQb = this.typeOrmRepository.manager
      .createQueryBuilder()
      .select([`COALESCE(SUM(loan.amount), 0) AS "historicalLent"`])
      .from('loans', 'loan')
    ...
    if (userId) {
      globalStatsQb.andWhere('loan.user_id = :userId', { userId });
      globalLentQb.andWhere('loan.user_id = :userId', { userId });
    }
    if (companyId) {
    ...
    const globalStatsRaw = await globalStatsQb.getRawOne<GlobalStatsRaw>();
    const globalLentRaw = await globalLentQb.getRawOne<GlobalLentRaw>();

    const historicalCollected = Number(
      globalStatsRaw?.historicalCollected ?? 0,
    );
    const historicalLent = Number(globalLentRaw?.historicalLent ?? 0);

    let thermometer = 0;
    if (historicalLent > 0) {
      const capitalRecovered = historicalCollected - historicalCollected * 0.2;
