import { Body, Controller, Delete, Get, Param, Post, Query, HttpStatus, HttpException } from '@nestjs/common';
...
export class ExpensesController {
    ...
    async register(@Body() dto: RegisterExpenseRequestDto) {
        const command = new RegisterExpenseCommand(dto.description, dto.amount, dto.userId);
    ...
    async findAll(
        ...
        const query = new ListExpensesQuery(userId, date ? new Date(date) : undefined);
        const expenses = await this.queryBus.execute(query);
        return expenses.map((expense: Expense) => new ExpenseResponseDto(expense));
    }
